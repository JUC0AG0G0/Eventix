...
export class EventService {
	...
	async unregisterUser(eventId: string, userId: string) {
		...
		// 1. On récupère l'événement pour vérifier son état actuel avant modification
		...
		// 2. Vérification de la règle "Status annulé -> 409 event_cancel"
		if (existing.Status === "Cancelled") {
			throw new ConflictException("event_cancel");
		}

		// 3. Vérifier si l'utilisateur est bien inscrit (pour éviter de décrémenter pour rien)
		const inscritIds = (existing.personneInscrites || []).map((id: any) => String(id));
		if (!inscritIds.includes(String(userObjectId))) {
		...
		// On retire l'utilisateur, on décrémente nbPlaceOccupe
		// On force le statut à "Ok" (car si c'était "Complet", une place se libère, donc ça devient "Ok")
		const update = {
			$pull: { personneInscrites: userObjectId },
			$inc: { nbPlaceOccupe: -1 },
			$set: {
				Status: "Ok", // Que ce soit Complet ou Ok avant, ça devient/reste Ok
				EditDate: new Date(),
			},
		};

		// On s'assure de ne modifier que si le status n'est PAS Cancelled (double sécurité)
		// et que l'user est bien dedans (sécurité concurrence).
		const updated = await this.eventModel
			.findOneAndUpdate(
				{
					_id: eventObjectId,
					Status: { $in: ["Ok", "Complet"] },
					personneInscrites: userObjectId,
				},
				update,
				{ new: true },
			)
		...
		if (!updated) {
			// Si on arrive ici, c'est que l'état a changé entre le findById et le update
			// ou que l'événement a été annulé entre temps.
			// On relance une vérif rapide pour renvoyer la bonne erreur
			const checkAgain = await this.eventModel.findById(eventObjectId).lean().exec();
			if (checkAgain && checkAgain.Status === "Cancelled") {
				throw new ConflictException("event_cancel");
			}
			throw new BadRequestException("Unable to unregister from event");
		}
